  ...
  return (
    <section>
      ...
      <div className="bg-[#3e362e] flex flex-col gap-3 py-[9.75rem] px-4 overflow-hidden w-full lg:px-[2.5rem]">
        <TextAnimate animation="slideLeft" className="text-[112px] tracking-[-6.8px] leading-[1em] text-[#AA8867] text-center">
          Explore Journal
        </TextAnimate>
        ...
        <div className="grid lg:grid-cols-3 lg:px-10 gap-6 mt-10">
          {data.ExploreJournal.journal.map((article, index) => (
            <motion.div
              ...
              transition={{ duration: 0.7, delay: index * 0.2, ease: 'easeInOut', type: 'spring', stiffness: 200 }}
              key={index}
              className="bg-white p-4"
            >
              <div className="w-full h-[24.25rem]">
                <motion.img src={article.titleimg} className="w-full h-full object-cover" />
              </div>
